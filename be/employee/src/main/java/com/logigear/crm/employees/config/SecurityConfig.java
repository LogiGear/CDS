...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    private static final String ALLOWED_EMPLOYEES_PATH = "/employees/api/employees/**";
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.cors().and().csrf().disable().authorizeRequests().antMatchers(ALLOWED_PATHS).permitAll()
                .antMatchers(ALLOWED_EMPLOYEES_PATH).permitAll().antMatchers(ALLOWED_SWAGGER).permitAll().anyRequest()
                .authenticated();
